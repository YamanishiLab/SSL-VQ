...
def load_gene_expression_dataset(args):
    ...
    if args.cell_name == "All":
        data = data.drop('cell_mfc_name', axis = 1).groupby(by = 'cmap_name').mean().reset_index() # Average gene expression signatures across all cell lines.
        ...
        data = data[ data['cell_mfc_name'] == args.cell_name].drop('cell_mfc_name', axis = 1) # Select cell line.
